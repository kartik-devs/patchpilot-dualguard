...
**Feedback loop lifts true fixes: 2/13 → 3–4/13 (3/13 robust across runs; 4/13 peak at N=2, Wilson 95% CI ≈ [12.7%, 57.6%]).** Feeding the gate's failing-PoV verdict back to the model as a retry signal (`N_RETRIES=2`) cleared two former silent fakes — VUL4J-50 (XSS) and VUL4J-41 (path-traversal) — flipping their PoVs FAIL→PASS while keeping compile, regression, and SAST green. The executable verdict is not just a filter; fed back, it teaches the model to actually close the hole.

**Robustness / variance.** Three fixes are robust under feedback — VUL4J-50, VUL4J-64, and VUL4J-77 cleared on **both** retry replications (`N_RETRIES` = 2 and 4); VUL4J-41 cleared at N=2 but not at N=4, i.e. variance at the margin. So with executable feedback the true-fix rate is **3/13 robustly and 4/13 at peak**, versus 2/13 single-shot — a consistent lift, not a clean monotonic curve. Notably VUL4J-50, a single-shot silent fake, becomes a *real* fix under feedback on every run. The precise count is noisy at N=13 with stochastic decoding; tighter intervals require a larger eval (future work).
